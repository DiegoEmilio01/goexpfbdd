...
func runLupConstVar(t *testing.T, id int, tc test.BTRecord, neg bool) {
	...
	var f test.Encodable = lup.ConstVar{c1, y, test.VarGenBotCount}
	if neg {
		f = logop.Not{Q: f}
	}
...
func runGuardedLupConstVar(t *testing.T, id int, tc test.BTRecord, neg bool) {
	tree, _ := test.NewMockTree(tc.Dim, nil)
	ctx := query.BasicQContext(tree)
	...
	var f test.Encodable = lup.ConstVar{x, y, test.VarGenBotCount}
	if neg {
		f = logop.Not{Q: f}
	}
...
func TestConstVar_Encoding_Guarded(t *testing.T) {
	for i, tc := range LupPTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedLupConstVar(t, i, tc, false)
		})
	}
}
...
func TestNotConstVar_Encoding_Guarded(t *testing.T) {
	for i, tc := range LupNTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedLupConstVar(t, i, tc, true)
		})
	}
}
...
func TestConstVar_Encoding_WrongDim(t *testing.T) {
	...
	f := lup.ConstVar{x, y, test.VarGenBotCount}
