...
func runSufConstConst(t *testing.T, id int, tc test.BTRecord, neg bool) {
	tree, _ := test.NewMockTree(tc.Dim, nil)
	ctx := query.BasicQContext(tree)

	var f test.Encodable = suf.ConstConst{
...
func runGuardedSufConstConst(
	...
	var f test.Encodable = suf.ConstConst{x, y}
	if neg {
		f = logop.Not{Q: f}
	}
...
func TestConstConst_Encoding_Guarded(t *testing.T) {
	for i, tc := range SufPTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedSufConstConst(t, i, tc, false)
		})
	}
}
