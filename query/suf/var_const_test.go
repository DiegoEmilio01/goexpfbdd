...
func runSufVarConst(t *testing.T, id int, tc test.BTRecord, neg bool) {
	...
	var f test.Encodable = suf.VarConst{I1: x, I2: c2}
	if neg {
		f = logop.Not{Q: f}
	}
...
func runGuardedSufVarConst(t *testing.T, id int, tc test.BTRecord, neg bool) {
	tree, _ := test.NewMockTree(tc.Dim, nil)
	ctx := query.BasicQContext(tree)
	...
	var f test.Encodable = suf.VarConst{I1: x, I2: y}
	if neg {
		f = logop.Not{Q: f}
	}
...
func TestVarConst_Encoding_Guarded(t *testing.T) {
	for i, tc := range SufPTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedSufVarConst(t, i, tc, false)
		})
	}
}
...
func TestNotVarConst_Encoding_Guarded(t *testing.T) {
	for i, tc := range SufNTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedSufVarConst(t, i, tc, true)
		})
	}
}
