...
func runSufConstVar(t *testing.T, id int, tc test.BTRecord, neg bool) {
	...
	var f test.Encodable = suf.ConstVar{I1: c1, I2: y}
	if neg {
		f = logop.Not{Q: f}
	}
...
func runGuardedSufConstVar(t *testing.T, id int, tc test.BTRecord, neg bool) {
	tree, _ := test.NewMockTree(tc.Dim, nil)
	ctx := query.BasicQContext(tree)
	...
	var f test.Encodable = suf.ConstVar{I1: x, I2: y}
	if neg {
		f = logop.Not{Q: f}
	}
...
func TestConstVar_Encoding(t *testing.T) {
	for i, tc := range SufPTT {
		t.Run(tc.Name, func(t *testing.T) {
			runSufConstVar(t, i, tc, false)
		})
	}
}
...
func TestConstVar_Encoding_Guarded(t *testing.T) {
	for i, tc := range SufPTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedSufConstVar(t, i, tc, false)
		})
	}
}

func TestNotConstVar_Encoding(t *testing.T) {
	for i, tc := range SufNTT {
		t.Run(tc.Name, func(t *testing.T) {
			runSufConstVar(t, i, tc, true)
		})
	}
}
...
func TestNotConstVar_Encoding_Guarded(t *testing.T) {
	for i, tc := range SufNTT {
		t.Run(tc.Name, func(t *testing.T) {
			runGuardedSufConstVar(t, i, tc, true)
		})
	}
}
